...
normFileList=spam.get_File_List(r"./dataset/trec06c/normal")
spamFileList=spam.get_File_List(r"./dataset/trec06c/spam")
testFileList=spam.get_File_List(r"./dataset/trec06c/test")
#获取训练集中正常邮件与垃圾邮件的数量
...
for fileName in normFileList:
    wordsList.clear()
    for line in open("./dataset/trec06c/normal/"+fileName,encoding='gbk'):
        #过滤掉非中文字符
...
for fileName in spamFileList:
    wordsList.clear()
    for line in open("./dataset/trec06c/spam/"+fileName,encoding='gbk'):
        rule=re.compile(r"[^\u4e00-\u9fa5]")
        line=rule.sub("",line)
        spam.get_word_list(line,wordsList,stopList)
    spam.addToDict(wordsList, wordsDict)
...
for fileName in testFileList:
    ...
    wordsDict.clear()
    wordsList.clear()
    for line in open("./dataset/trec06c/test/"+fileName,encoding='gbk'):
        rule=re.compile(r"[^\u4e00-\u9fa5]")
        line=rule.sub("",line)
        spam.get_word_list(line,wordsList,stopList)
    spam.addToDict(wordsList, wordsDict)
